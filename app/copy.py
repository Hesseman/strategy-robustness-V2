...
CARDS = {
    ...
    "wfc": {
        ...
        "how": "Gate - the region lift, per walk-forward window, on net profit. Each combination's in-sample result is averaged with its one-step neighbours on the grid (the pooled surface); the top 20% of that pooled surface is the in-sample region. Lift L = (the region's mean out-of-sample net profit - the grid's) / the spread of out-of-sample net profit across the grid, in SDs, on the raw out-of-sample results. The null re-draws the out-of-sample surface by flipping the sign of each 21-trading-day block's deviations from the grid's daily average - the strategy's common path and how alike neighbouring combinations are both stay exactly as they were, only the alignment with in-sample is broken; p = (draws at least as large + 1) / (draws + 1), pooled over the complete windows. Verdict - Tinsley's matrix with the lift as the row test: lift significant (p < 0.05) and the region profitable out-of-sample - PASS, a structural edge localised in the region; significant but the region loses - FAIL, a consistent loser; not significant while the grid as a whole made money out-of-sample - PLATEAU: the parameter choice is immaterial and the strategy-level cards govern (gate not applied); not significant and the grid lost - FAIL, noise. A grid whose out-of-sample results repeat (fewer distinct patterns than twice the region: identical variants) is not scored: PLATEAU. Every PASS is printed with what to read it with: the lift's p under a second null - a centred block bootstrap that resamples the same 21-day blocks instead of flipping them, because the flip assumes a block's deviations are as likely as their negation and a few large divergent trades can break that - flagged when one p is below 0.05 and the other is not (the flip stays the gate); the effective number of independent variants (from the selection card; below 3 the variants are nearly one strategy and the lift rests on the few trades where they differ - context, not a gate, since the null holds however alike they are), the region's and the grid's out-of-sample dollars, and the region's margin over the grid in each window - a pass carried by one window reads differently from one that holds in all. No minimum trade count per combination - the null already prices each one's noise; below about 20 out-of-sample trades per combination nothing has power, and 2 windows (thirds) give longer blocks. Also shown: the overlap (share of the region inside the out-of-sample top 20% of the pooled surface; chance is 20%), the ridge (largest connected top-20% area) in vs out-of-sample with its overlap and how far its centre moved, where four picks landed out-of-sample, and Tinsley's own correlation - Spearman over every combination on the project's fitness, same null - for continuity, never as the gate. Tabs: Surface (heatmaps in and out-of-sample, raw and pooled, each ridge outlined; with 3+ parameters the slice through the best pooled in-sample combination); Scatter (one dot per combination, with the green best-fit line of Tinsley's chart); Ranked profile (combinations sorted by in-sample result, best on the left - the in-sample rank is the falling blue line, the out-of-sample rank of the same combination an orange dot, hollow when it lost money); Bands (grids of 100+ combinations: the mean out-of-sample result per tenth of the in-sample ranking); the two nulls. The top in-sample combinations are outlined in red, and each window's line says where they landed out-of-sample.",
    },
    "base": {
        "title": "Base setting - which combination to trade?",
        "tagline": "The best region of the grid, not its luckiest cell - its pooled peak after an edge, its centre on a plateau",
        "catches": "The single best in-sample combination is the one most likely to owe its place to luck, and its neighbours may be much worse. A combination in the middle of a region that did well as a whole keeps working when the market drifts a little; the peak often does not.",
        "how": "Basis: the last walk-forward window's in-sample plus its out-of-sample to date (the later half of the history on the 1-split scheme) - one cycle of the schedule the WFC card tested, recent enough that the region has not moved. Each combination's net profit over the basis is averaged with its one-step neighbours; the region is the largest connected area in the top 20% of that pooled surface. After a PASS the base setting is the pooled peak - the best neighbourhood average, which on the real edges tested held up best out of sample - and the region's centre and ensemble are the diversified alternative; on a plateau, and for reference otherwise, it is the region's combination nearest its centre (Kaufman's rule: trade the middle of the best settings; with fewer than 3 combinations there is no meaningful centre, and the pooled peak stands in). The ensemble spreads up to 4 members across the region - the best pooled combinations at least 2 steps from each other, preferring cells whose neighbours are all inside the region - one member per 3 region cells, each run at 1/k size. Confidence follows the WFC verdict: SUPPORTED after an edge, LOW STAKES on a plateau (any setting in the region does about as well - take the centre), NO RECOMMENDATION after noise or a consistent loser. The evidence for the rule is scored window by window without hindsight: the centre chosen in-sample, its out-of-sample percentile next to the best in-sample combination, the pooled peak, the whole region and the window's pick. Kaufman's literal average of the five best is shown for comparison only - when those five sit in separate areas their average lands in the valley between them. Dollar or point parameters are labelled price-scaled and used as tested: on the real grids checked, their optima stayed steadier in dollars than when scaled by volatility.",
    },
    "plateau": {
...
**Optional: MultiWalk surface tests** (bottom of the page) read every parameter combination of a MultiWalk optimisation and ask whether the region of the grid that was best in-sample beat the grid average out-of-sample (WFC), which combination to trade (the base setting - the centre of that region, not its peak), whether the chosen parameters sit on a plateau, and how much of the best in-sample result is selection luck."""

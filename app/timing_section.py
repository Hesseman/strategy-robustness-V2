...
def _report(report_bytes: bytes):
...
def _timing(report_bytes: bytes, bars_bytes: bytes, max_k: int, mode: str) -> TimingResult:
    return run_timing(_report(report_bytes), _bars(bars_bytes), max_k=max_k, mode=mode)
...
def render(report_bytes: bytes, bars_bytes: bytes) -> None:
    ...
    a JSON download. Returns nothing. Guarantees it never calls st.stop() - the MultiWalk
    section below must still render; an unexpected failure shows one warning and returns."""
    ...
    max_k = c1.slider("Largest shift, bars", min_value=1, max_value=20, value=10, step=1,
                      key="timing_max_k", help=HELP["timing_max_k"])
    mode = c2.radio("Delayed entry", options=list(MODE_LABELS), format_func=MODE_LABELS.get,
                    key="timing_mode", horizontal=True, help=HELP["timing_mode"])
    try:
    ...
    m = result.meta
    st.caption(_esc(f"{m['n_trades']} trades · shifts 0..{m['max_k']} bars each way · "
                    f"mode: {MODE_LABELS[m['mode']].lower()} · 1 contract at {m['point_value']:g} USD/pt, gross"))
    ...
    st.download_button("Download timing results (JSON)", data=to_json(result),
                       file_name="timing_results.json", mime="application/json")
